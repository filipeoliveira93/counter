...
btns.forEach((btn) => { // para cada botão adiciona um escutador de evento
    btn.addEventListener('click', (item) => { //ao ser clicado vai chamar uma função
        const style = item.currentTarget.classList //pega a listta de classes do item
        if (style.contains('decrement')) { //verifica se no objeto style tem a string decrement
            count--
        } else if (style.contains('increment')) { //verifica se no objeto style tem a string increment
            count++}
        else { //verifica se no objeto style tem a string reset
            count = 0
        }
